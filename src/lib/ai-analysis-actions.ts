...
用户身份：
Isabella，26届数字媒体艺术应届生，正在转向 AI 产品经理。

产品背景：
Second AI 是一个可控访问的 AI 作品主页生成工具。用户上传资料后，AI 需要理解资料能证明什么能力、适合回答哪些问题、是否适合在访客主页中展示。

资料信息：
- file_name: ${asset.file_name}
- file_type: ${asset.file_type}
- original_text/raw_text: ${asset.raw_text || "当前没有提取正文。本阶段请根据文件名、文件类型和已有摘要谨慎推断，不要编造具体经历。"}
    ...
    messages: [
      {
        role: "system",
        content:
          "你是 Second AI 的资料解析器。你只返回严格 JSON，不返回 Markdown，不编造资料正文中不存在的具体事实。",
      },
      {
        role: "user",
...
export async function analyzeAssetAction(formData: FormData) {
  ...
  try {
    const analysis = await runOpenAiAnalysis(asset as AssetRecord);
    const suggestedVisibility = analysis.suggested_visibility === "show" ? "display" : analysis.suggested_visibility;
    const nextVisibility = asset.visibility || suggestedVisibility;
